...
def predict(image_path, model, topk=5):
    ...
    probs = probs.detach().np()[0]

    classes = classes.np()[0]

    classes = [model.classifier.class_to_idx[str(i)] for i in classes]
...
def plot_solution(test_image, model):
    
    plt.figure(figsize = (6,10))
    ax = plt.subplot(2,1,1)
    
    flower_num = image_path.split('/')[1]
    title = label_name[flower_num]
...
plot_solution(test_image, model)
